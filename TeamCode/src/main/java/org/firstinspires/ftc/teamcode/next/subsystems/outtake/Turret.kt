...
import org.firstinspires.ftc.teamcode.next.subsystems.Outtake.spin
...
object Turret: Subsystem {
    private val tele =
        MultipleTelemetry(FtcDashboard.getInstance().telemetry, ActiveOpMode.telemetry)
    ...
    private fun autoAim() {
        ...
        val goalHeading = (mu - currentHeading).rad.normalized.inRad
        val clampedHeading = shiftInterval(goalHeading) // From (-180, 180]
    ...
    fun shiftInterval(ang: Double): Double {
        return if (ang <= -PI) ang + 2 * PI else if (ang > PI) ang - 2 * PI else ang
    }
}